...
# Display something on the LCD for the next 35 seconds.

mylcd = I2C_LCD_driver.lcd()
mylcd.lcd_display_string("CO2 Monitor", 1)
mylcd.lcd_display_string("AHS AQ Club", 2)
...
# Measure core temperature of the RPi3B+
        ...
        ts = [time_stamp]
        mylist = [''.join(ts),
                  round(res['gt03um'],2), round(res['gt05um'],2), 
                  round(res['gt10um'],2), round(res['gt25um'],2), 
                  round(res['gt50um'],2), round(res['gt100um'],2),
                  res['pm10'], res['apm25'], res['pm100'],
                  round(degrees,2),  round(hectopascals,2), 
                  round(humidity,2),  round(conc_CO2,2),  core_T]
        wr.writerow(mylist)
#       print(mylist)
...
# Write to LCD
        ...
        mylcd.lcd_clear()
        mylcd.lcd_display_string("CO2 = %d ppm" % conc_CO2, 1)
        mylcd.lcd_display_string("T=%d%sF RH=%d%%" 
             % (degrees, chr(223), humidity), 2)

#Sleep 10 seconds (5 with MH-Z19 and 5 here). 
